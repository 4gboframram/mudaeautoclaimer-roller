            ...
            try:
                ...
                if len(t)<=1: #basically if there's either only hours or only minutes
                    ...
                    if 'h' in message.content: 
                        print(colored(f"[{current_time}] Next $dk in '{chanel.name}' in {t[0]}h 0m. Asyncronously sleeping in '{chanel.name}' that amount of time.",'cyan'))
                        await asyncio.sleep(3600*t[0])
                    if 'min' in message.content: 
                        print(colored(f"[{current_time}] Next $dk in '{chanel.name}' in 0h {t[0]}m. Asyncronously sleeping in '{chanel.name}' that amount of time.",'cyan'))
                        await asyncio.sleep(60*t[0])
                print(colored(f"[{current_time}] Next $dk in '{chanel.name}' in {t[0]}h {t[1]}m. Asyncronously sleeping in '{chanel.name}' that amount of time.",'cyan'))
    ...
    try:
        ...
        if title in characters or any(series in description for series in Series): 
            ...
            try:
                ...
                if message.reactions[0].emoji.id==469835876265099275: #:wleft: emoji id
                    current_time = time.strftime("%D %H:%M:%S", time.localtime())
